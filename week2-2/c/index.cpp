...
int main()
{
    ...
    for (unsigned int offset = 0; offset < 32; offset += 16)
    {
        ...
        for (unsigned int i = 0; i < n; ++i) ++counter[(data[i].second >> offset) & 65535];
        for (int i = 65534; i >= 0; --i) counter[i] += counter[i + 1];
        for (int i = n - 1; i >= 0; --i) res[--counter[(data[i].second >> offset) & 65535]] = data[i];
        std::swap(data, res);
    }
