...
private:
    ...
    Node(const K& key, const V& value, Node<K, V>* parent) : data_({ key, value }), key(key), value(value), parent(parent) { };
    Node(const Node<K, V>& other, Node<K, V>* parent) : data_({ other.key, other.value }), key(other.key), value(other.value), parent(parent), color(other.color)
    {
        if (other.left) this->left = new Node<K, V>(*other.left, this);
        if (other.right) this->right = new Node<K, V>(*other.right, this);
    }
...
private:
    ...
    void fix_balance(Node<K, V>* current);
    void fix_double_black(Node<K, V>* current);
...
public:
    ...
    Map() = default;
    Map(std::initializer_list< std::pair<K, V> > list) { for (const std::pair<K, V>& data : list) this->insert(data.first, data.second); } // O(nlog(n))
    ...
    ~Map() { this->destroy(this->root); } // O(n)
    ...
    public: 
        Iterator() = default;
        explicit Iterator(Node<K, V>* node) : node(node) { }

        const std::pair<K, V>& operator*() const { return this->node->data_; }
        const std::pair<K, V>* operator->() const { return &(this->node->data_); }
...
template <typename K, typename V>
void Map<K, V>::fix_balance(Node<K, V>* current)
{
    ...
    Node<K, V>* grandparent = parent->parent; // definitely exists because parent is red (thus, not root)
    Node<K, V>* uncle = parent->isLeftChild() ? grandparent->right : grandparent->left; // may not exist
...
{
    if (this->length == 0) { this->root = new Node<K, V>(key, value); ++this->length; this->fix_balance(this->root); return; }
    Node<K, V>* current = this->root;
    while (key < current->key || current->key < key) // key != current->key
    {
        if (key < current->key)
        {
            if (!current->left) { current->left = new Node<K, V>(key, value, current); ++this->length; this->fix_balance(current->left); return; }
            current = current->left;
        }
        else
        {
            if (!current->right) { current->right = new Node<K, V>(key, value, current); ++this->length; this->fix_balance(current->right); return; }
...
template <typename K, typename V>
void Map<K, V>::fix_double_black(Node<K, V>* current)
{
    ...
    Node<K, V>* sibling = current->isLeftChild() ? parent->right : parent->left; // definitely exists
...
void Map<K, V>::erase(const K& key)
{
    ...
    if (!current || key < current->key || current->key < key) return; // !current || key != current->key
    this->length--;
...
{
    Node<K, V>* answer = nullptr; Node<K, V>* current = this->root;
    while (current)
    {
        if (!(key < current->key || current->key < key)) return Map<K, V>::Iterator(current); // key == current->key
...
{
    Node<K, V>* current = this->root;
    if (!current) return Map<K, V>::Iterator(nullptr);
    while (current->left) current = current->left;
    return Map<K, V>::Iterator(current);
}
template <typename K, typename V>
typename Map<K, V>::Iterator Map<K, V>::end() const { return Map<K, V>::Iterator(nullptr); }
...
template <typename K, typename V>
typename Map<K, V>::Iterator Map<K, V>::Iterator::operator++(int) { Map<K, V>::Iterator save = *this; ++(*this); return save; }
