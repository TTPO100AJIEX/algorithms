...
void Deque::pushBack(int value)
{
    ...
    if ((this->size() & 1) == 1) { this->middle_iterator_++; return; }
    if (this->size() == 2) { this->middle_iterator_ = this->storage_.end(); this->middle_iterator_--; return; }
}
void Deque::pushMiddle(int value)
{
    this->storage_.insert(this->middle_iterator_, value);
    if ((this->size() & 1) == 0) this->middle_iterator_--;
}
...
int Deque::popFront()
{
    ...
    if ((this->size() & 1) == 1) this->middle_iterator_++;
    return (item);
}
