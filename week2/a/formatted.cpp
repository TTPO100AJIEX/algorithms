...
const Vector::Iterator::DifferenceType operator-(Vector::Iterator left,
...
void Vector::resize(size_t n_size) {
    ...
    if (n_size > this->size_) {
        for (size_t i = this->size_; i < n_size; i++) {
            this->arr_[i] = 0;
        }
    }
    this->size_ = n_size;
...
void Vector::erase(size_t pos) {
    ...
    this->size_--;
    for (; pos < this->size_; pos++) {
        std::swap(this->arr_[pos], this->arr_[pos + 1]);
    }
}
