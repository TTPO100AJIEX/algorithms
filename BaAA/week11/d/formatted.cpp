...
int main() {
    ...
    std::vector<std::vector<unsigned int> > distance(n, std::vector<unsigned int>(n, kDistanceMax));
    ...
    while (distance[x2][y2] == kDistanceMax) {
        Vertex current = to_update.front();
        to_update.pop();

        Vertex siblings[8] = {
            {current.first + 2, current.second + 1}, {current.first + 2, current.second - 1},
            {current.first - 2, current.second + 1}, {current.first - 2, current.second - 1},
            {current.first + 1, current.second + 2}, {current.first + 1, current.second - 2},
            {current.first - 1, current.second + 2}, {current.first - 1, current.second - 2}};
        for (const Vertex& sibling : siblings) {
            if (sibling.first < 0 || sibling.second < 0 || sibling.first >= n ||
                sibling.second >= n || distance[sibling.first][sibling.second] != kDistanceMax) {
                continue;
            }
    ...
    for (std::vector<Vertex>::const_reverse_iterator iter = path.rbegin(); iter != path.rend();
         ++iter) {
