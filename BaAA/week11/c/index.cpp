...
void print(unsigned int current)
{
    ...
    std::sort(people[current].siblings.begin(), people[current].siblings.end(), [](unsigned int first, unsigned int second) { return people[first].name < people[second].name; });
    for (unsigned int sibling : people[current].siblings) if (!people[sibling].name.empty()) print(sibling);
}
