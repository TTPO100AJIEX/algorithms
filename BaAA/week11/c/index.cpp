...
bool personIndexComparator(const unsigned int first, const unsigned int second) { return names[first] < names[second]; }
std::vector<std::vector<unsigned int>> edges;

void print(unsigned int current)
{
    if (names[current].empty()) return;
    std::cout << names[current] << '\n';
    names[current].clear();
    for (unsigned int sibling : edges[current])
    {
        if (!names[sibling].empty()) print(sibling);
    }
}
