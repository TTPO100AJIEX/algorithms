...
std::set<unsigned int> search(std::string_view text, std::string_view pattern)
{
    ...
    for (std::string_view part : parts)
    {
        ...
        std::vector <unsigned int> patternBlocks(pattern.size()), patternBlocksSpecific(pattern.size());
        ...
        */
        ...
        {
            while (matched != 0 && part[matched] != text[i]) matched = patternBlocksSpecific[matched - 1];
