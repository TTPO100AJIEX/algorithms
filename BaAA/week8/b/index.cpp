...
struct Trie
{
    ...
    void add(char letter)
    {
        char code;
        std::cin >> code;
        if (code == '\n') { this->letter = letter; return; }
        if (code == '1')
        {
            if (this->left == nullptr) this->left = std::make_unique<Trie>();
            this->left->add(letter);
        }
        else
        {
            if (this->right == nullptr) this->right = std::make_unique<Trie>();
...
int main()
{
    ...
    {
        if (encoded[i] == '1') current = current->left.get();
