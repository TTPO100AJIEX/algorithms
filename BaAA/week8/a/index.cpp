...
struct Node
{
    ...
    Node(unsigned int weight, std::unique_ptr<Node>& left, std::unique_ptr<Node>& right) : weight(weight), left(std::move(left)), right(std::move(right)) { }
...
int main()
{
    ...
    unsigned int counter[symbolsSize];
    for (unsigned int i = 0; i < symbolsSize; ++i) counter[i] = 0;
    ...
    while (nodes.size() != 1)
    {
        std::sort(nodes.begin(), nodes.end(), [](const std::unique_ptr<Node>& first, const std::unique_ptr<Node>& second)
        {
    ...
    std::vector<std::string> codes(symbolsSize, "");
    nodes[0]->buildCodes(codes);
    std::string encoded = "";
    for (unsigned int i = 0; i < data.size(); ++i) encoded += codes[data[i] - 'a'];
