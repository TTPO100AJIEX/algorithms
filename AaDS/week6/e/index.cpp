...
class Set
{
    ...
    void fix_balance(Node* current);
    void fix_double_black(Node* current);
    void recalculate_size(Node* current);
...
void Set::fix_balance(Node* current)
{
    if (!current) return;
    current->recalculateSize();
    if (this->root == current) { current->color = Node::Color::BLACK; return; }
    if (current->color == Node::Color::BLACK || current->parent->color == Node::Color::BLACK) { this->recalculate_size(current->parent); return; }
    ...
    if (uncle && uncle->color == Node::Color::RED)
    {
        ...
        if (uncle) uncle->color = Node::Color::BLACK;
    }
    else
    {
        // repaint + rotation
...
void Set::fix_double_black(Node* current)
{
    if (current == this->root) { current->color = Node::Color::BLACK; current->is_double_black = false; return; }
    Node* parent = current->parent; // definitely exists
    Node* sibling = current->isLeftChild() ? parent->right : parent->left; // definitely exists
    if (sibling->color == Node::Color::BLACK && (!sibling->left || sibling->left->color == Node::Color::BLACK) && (!sibling->right || sibling->right->color == Node::Color::BLACK)) // Case 3
    {
        sibling->color = Node::Color::RED; current->is_double_black = false;
        if (parent->color == Node::Color::RED) parent->color = Node::Color::BLACK; // Case 3.1
        else parent->is_double_black = true; // Case 3.2
        if (parent->is_double_black) this->fix_double_black(parent);
        return;
    }
    if (sibling->color == Node::Color::RED && (!sibling->left || sibling->left->color == Node::Color::BLACK) && (!sibling->right || sibling->right->color == Node::Color::BLACK)) // Case 4
    {
        this->rotation(parent, current->isLeftChild() ? Rotations::LEFT : Rotations::RIGHT);
        parent->color = Node::Color::RED; sibling->color = Node::Color::BLACK;
        this->fix_double_black(current);
        return;
    }

    // sibling is definitely black
    if ((current->isLeftChild() && sibling->right && sibling->right->color == Node::Color::RED) || (current->isRightChild() && sibling->left && sibling->left->color == Node::Color::RED)) // Case 6
    {
        this->rotation(parent, current->isLeftChild() ? Rotations::LEFT : Rotations::RIGHT);
        sibling->color = parent->color; parent->color = Node::Color::BLACK;
        
        if (current->isLeftChild()) sibling->right->color = Node::Color::BLACK;
        else sibling->left->color = Node::Color::BLACK;
        current->is_double_black = false;
    }
    else // Case 5 (sibling->left->color == Color::RED) || (sibling->right->color == Color::RED)
    {
        if (current->isLeftChild()) sibling->left->color = Node::Color::BLACK;
        else sibling->right->color = Node::Color::BLACK;
...
void Set::recalculate_size(Node* current)
{
    if (!current) return;
    current->recalculateSize();
    this->recalculate_size(current->parent);
}
...
{
    if (!this->root) { this->root = new Node(value); this->fix_balance(this->root); return; }
...
void Set::erase(int value)
{
    ...
    if (current->left && !current->right)
    {
        if (!parent) { this->root = current->left; current->left->parent = nullptr; delete current; this->recalculate_size(this->root); this->root->color = Node::Color::BLACK; return; }
        if (current->isLeftChild()) parent->left = current->left;
        else parent->right = current->left;
        current->left->parent = parent;
        if (current->color == Node::Color::BLACK) current->left->color = Node::Color::BLACK;
        delete current;
        this->recalculate_size(parent);
        return;
    }
    if (!current->left && current->right)
    {
        if (!parent) { this->root = current->right; current->right->parent = nullptr; delete current; this->recalculate_size(this->root); this->root->color = Node::Color::BLACK; return; }
        if (current->isLeftChild()) parent->left = current->right;
        else parent->right = current->right;
        current->right->parent = parent;
        if (current->color == Node::Color::BLACK) current->right->color = Node::Color::BLACK;
        delete current;
        this->recalculate_size(parent);
        return;
    }
